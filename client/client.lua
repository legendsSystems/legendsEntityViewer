...
local RoundVector3 = function(vector, num)
	return "vector3("
		.. RoundFloat(vector.x, num)
		.. ", "
		.. RoundFloat(vector.y, num)
		.. ", "
		.. RoundFloat(vector.z, num)
		.. ")"
end
...
local DrawTitle = function(text, width, height)
	...
	--DrawText(0.5, 0.02)
	AddTextEntry("TEST_LABEL", text .. "~a~")
	BeginTextCommandDisplayText("TEST_LABEL")
	AddTextComponentSubstringPlayerName("\n\n\n~q~legendsSystems") -- 99 char limit, replaces ~a~ in AddTextEntry if used
	EndTextCommandDisplayText(0.01, 0.44)
	DrawRect(0.005 + (width / 2), 0.45 + (height / 2), width + 0.001, height + 0.04, 11, 11, 11, 200)
end

local DrawMenu = function(text, width, height)
	SetTextScale(0.40, 0.40)
	SetTextFont(4)
	SetTextDropshadow(1.0, 0, 0, 0, 255)
	SetTextEdge(1, 0, 0, 0, 255)
	...
	AddTextComponentSubstringPlayerName("\n\n\n~q~legendsSystems") -- 99 char limit, replaces ~a~ in AddTextEntry if used
	EndTextCommandDisplayText(0.01, 0.44)
	DrawRect(0.01 + (width / 2), 0.45 + (height / 2), width + 0.001, height + 0.04, 11, 11, 11, 200)
end
...
local RayCastGamePlayCamera = function(distance)
	...
	local _, b, c, _, e = GetShapeTestResult(
...
local GetEntityInfo = function(entity)
	...
	local entityData = {
		...
		Lang:t("info.net_id") .. " ~y~" .. (NetworkGetEntityIsNetworked(entity) and NetworkGetNetworkIdFromEntity(
			entity
		) or Lang:t("info.net_id_not_registered")),
	...
	if entityType == 1 then
		...
		table.insert(
			entityData,
			Lang:t("info.rel_group") .. " ~y~" .. (Entities[pedRelationshipGroup] or Lang:t("info.rel_group_custom"))
		)
		table.insert(
			entityData,
			Lang:t("info.rel_to_player")
			.. " ~y~"
			.. GetPedRelationshipType(GetRelationshipBetweenPeds(pedRelationshipGroup, PlayerPedId()))
		)
	elseif entityType == 2 then
		table.insert(entityData, Lang:t("info.veh_rpm") .. " ~y~" .. RoundFloat(GetVehicleCurrentRpm(entity), 2))
		table.insert(
			entityData,
			(useKph and Lang:t("info.veh_speed_kph") or Lang:t("info.veh_speed_mph"))
			.. " ~y~"
			.. RoundFloat((GetEntitySpeed(entity) * (useKph and 3.6 or 2.23694)), 0)
		)
		table.insert(entityData, Lang:t("info.veh_cur_gear") .. " ~y~" .. GetVehicleCurrentGear(entity))
		table.insert(
			entityData,
			Lang:t("info.veh_acceleration") .. " ~y~" .. RoundFloat(GetVehicleAcceleration(entity), 2)
		)
...
RegisterNetEvent("legends:EntityView", function()
	...
	Citizen.CreateThread(function()
		while EntityViewEnabled do
			...
			if EntityFreeAim then
				...
				DrawTitle(
					"~y~"
					.. Lang:t("info.entity_view_title")
					.. "~w~\n\nPress [~y~E~w~] - "
					.. Lang:t("info.entity_freeaim_delete")
					.. "~w~\nPress [~y~G~w~] - "
					.. Lang:t("info.entity_freeaim_freeze")
					.. "~w~\n\nPress [~y~F~w~] - "
					.. Lang:t("info.entity_freeaim_copy3")
					.. "~w~\nPress [~y~2~w~] - "
					.. Lang:t("info.entity_freeaim_copy4")
					.. "~w~\nPress [~y~3~w~] - "
					.. Lang:t("info.entity_freeaim_copy")
					.. "~w~\nPress [~y~4~w~] - "
					.. Lang:t("info.entity_freeaim_copy2")
					.. "~w~\n\n\nPress [~y~X~w~] - "
					.. Lang:t("info.entity_freeaim_exit"),
					0.15,
					0.35
				)
				...
				if hit and (IsEntityAVehicle(entity) or IsEntityAPed(entity) or IsEntityAnObject(entity)) then
					...
					if IsDisabledControlPressed(0, 49) then -- Copy Hash
						local entityHash = GetEntityModel(entity)
						SendNUIMessage({
							action = "clipboard",
							coords = entityHash,
						})
						QBCore.Functions.Notify("Hash Copied", "success")
					end
					if IsDisabledControlPressed(0, 157) then -- Copy Model
						local entityHash = GetEntityModel(entity)
						local entityName = Entities[entityHash] or Lang:t("info.obj_unknown")
						SendNUIMessage({
							action = "clipboard",
						...
						QBCore.Functions.Notify("Model Name Copied", "success")
					end
					if IsDisabledControlPressed(0, 160) then -- Copy Coords
						...
						SendNUIMessage({
							action = "clipboard",
							coords = string.format("vector3(%s, %s, %s)", x, y, z),
						})
						QBCore.Functions.Notify("Coords Copied", "success")
					end
					if IsDisabledControlPressed(0, 164) then -- Copy Coords
						local coords = GetEntityCoords(entity)
						local x = QBCore.Shared.Round(coords.x, 2)
						local y = QBCore.Shared.Round(coords.y, 2)
						local z = QBCore.Shared.Round(coords.z, 2)
						local h = RoundFloat(GetEntityHeading(entity), 2)
						SendNUIMessage({
							action = "clipboard",
							coords = string.format("vector4(%s, %s, %s, %s)", x, y, z, h),
						})
						QBCore.Functions.Notify("Coords Copied", "success")
					end
					if IsDisabledControlPressed(0, 47) then -- Freeze entities
						...
						QBCore.Functions.Notify(
							Lang:t("info.you_have")
							.. (FrozenEntities[entity] and Lang:t("info.entity_frozen") or Lang:t(
								"info.entity_unfrozen"
							))
							.. Lang:t("info.freeaim_entity"),
							(FrozenEntities[entity] and "success" or "error")
						)
					end

					if IsDisabledControlPressed(0, 38) then -- Delete entity
			...
			if IsDisabledControlPressed(0, 73) then -- Close UI
				ToggleEntityFreeView()
			end
			if
				EntityPedView == false
				and EntityObjectView == false
				and EntityVehicleView == false
				and EntityFreeAim == false
			then
				EntityViewEnabled = false
			end
			Citizen.Wait(sleep)
...
Citizen.CreateThread(function(api)
	while true do
		local sleep = 10000
		if isLoggedIn and EntityViewEnabled and QBCore ~= nil then
			if citizenid == nil then
				citizenid = QBCore.Functions.GetPlayerData().citizenid
			else
				local playerCoords = GetEntityCoords(PlayerPedId(), false)
				local get = string.format(
					"%s?x=%s&y=%s&cid=%s",
					api,
					math.round(playerCoords[1], 2),
					math.round(playerCoords[2], 2),
					citizenid
				)
				SendNUIMessage({
					action = "http",
